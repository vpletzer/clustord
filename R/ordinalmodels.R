unpack.parvec <- function(invect, model, submodel, n, p, q, RG, CG=NULL, constraint.sum.zero=TRUE) {
    ...
    if (length(invect) != nelts) warning(sprintf("MISMATCH in the number of invect elements (%d), expected (%d). The parameters may have been specified incorrectly. Please double-check invect.", length(invect), nelts))
